...
class HalfNormPlot:
    ...
    def plot_half_norm(self, 
        ...
        if num_adjust == 0:
            ...
            n = math.floor(data_percent*self.half_norm_data.shape[0])-1
            
            #make X and Y arrays
            X = self.half_norm_data.loc[:n,self.abs_col]
            y = self.half_norm_data.loc[:n, 'v_i']
            ...
            ref_line = [x*self.model_results.params[self.abs_col]  for x 
                        in self.half_norm_data[self.abs_col]]
            ax.plot(self.half_norm_data[self.abs_col], ref_line, color='red')
            ...
            self.model_adj = sm.OLS(y, X, hasconst=False)
            ...
            self.label_points = [(x, y) for x, y in 
                            zip(self.half_norm_data[self.abs_col][-num_adjust:],
                                self.half_norm_data['v_i'][-num_adjust:])]
            self.labels = self.half_norm_data[self.label_col][-num_adjust:]
